...
export class IntegrationsApiResponseProcessor {
     ...
     */
     public async listIntegrationsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ListIntegrations200Response >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: ListIntegrations200Response = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ListIntegrations200Response", ""
            ) as ListIntegrations200Response;
