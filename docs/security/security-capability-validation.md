...
| NIRVANA Agent Workload Identity | **Planned** (hardening path) | Future: SPIRE Workload API |
...
**CAP-004** in the capability evidence has status `planned` for "SPIFFE/SPIRE workload identity." This refers to the **fuller NIRVANA-style agent workload identity model** with SPIRE Workload API integration.
...
- `src/saas/customer-api/evidence/evidence.go:127` — CAP-004 planned (NIRVANA model)
